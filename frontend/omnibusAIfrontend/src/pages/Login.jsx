...
export default function Login() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      setAuth(res.data.token, { email: res.data.email, name: res.data.name });
      navigate('/dashboard');
    }
    catch (err) {
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.5, ease: "easeOut" }} className="w-full max-w-[420px] z-10">
        <Card className="border-white/20 dark:border-neutral-800/50 shadow-2xl backdrop-blur-2xl bg-white/70 dark:bg-neutral-900/70 rounded-[2.5rem] p-6 md:p-10 relative overflow-hidden">
          ...
          <CardHeader className="space-y-3 text-center pb-8 pt-4 px-0">
            <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: "spring", delay: 0.2 }} className="w-16 h-16 bg-blue-600 rounded-2xl mx-auto flex items-center justify-center shadow-lg shadow-blue-500/30 mb-4 transform rotate-3">
          ...
          <CardContent className="px-0 pb-0">
            <motion.form variants={containerVariants} initial="hidden" animate="show" onSubmit={handleLogin} className="space-y-6">
              ...
              <motion.div variants={itemVariants} className="space-y-2">
                ...
                <div className="relative group">
                  ...
                  <Input
                    ...
                    required
                    className="pl-11 h-12 text-sm rounded-2xl bg-white dark:bg-neutral-950 border-neutral-200 dark:border-neutral-800 focus-visible:ring-blue-500 shadow-sm transition-all"
                  />
                </div>
              </motion.div>
              ...
              <motion.div variants={itemVariants} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} className="pt-2">
                <Button type="submit" disabled={isLoading || !email || !password} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-semibold text-base h-12 rounded-2xl shadow-xl shadow-blue-500/20 transition-all relative overflow-hidden group">
                  {isLoading ? (
